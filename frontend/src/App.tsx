        ...
        {showRulesModal && (
          <div className="modal-overlay">
            <div className="rules-modal">
              <div className="modal-header"><Zap size={32} color="var(--primary)" /> <h2>{t('rules_title')}</h2></div>
              <div className="rules-grid">
                <div className="rule-item">
                  <Target size={28} color="var(--accent)" /> 
                  <div><h4>{t('rule_base_title')}</h4><p dangerouslySetInnerHTML={{ __html: t('rule_base_desc') }}></p></div>
                </div>
                <div className="rule-item">
                  <Clock size={28} color="var(--primary)" /> 
                  <div><h4>{t('rule_time_title')}</h4><p dangerouslySetInnerHTML={{ __html: t('rule_time_desc') }}></p></div>
                </div>
                <div className="rule-item">
                  <Zap size={28} color="#4ade80" /> 
                  <div><h4>{t('rule_speed_title')}</h4><p dangerouslySetInnerHTML={{ __html: t('rule_speed_desc') }}></p></div>
                </div>
                <div className="rule-item">
                  <Flame size={28} color="#fb923c" /> 
                  <div><h4>{t('rule_streak_title')}</h4><p dangerouslySetInnerHTML={{ __html: t('rule_streak_desc') }}></p></div>
                </div>
                <div className="rule-item">
                  <Compass size={28} color="var(--accent)" /> 
                  <div><h4>{t('rule_discovery_title')}</h4><p dangerouslySetInnerHTML={{ __html: t('rule_discovery_desc') }}></p></div>
                </div>
              </div>
              <button className="primary-action-btn" onClick={() => { setShowRulesModal(false); startCompetition(); }}><Play size={20} fill="currentColor" /> {t('rules_start')}</button>
            </div>
          </div>
        )}
